...
@section('title', 'Edit Brand - ' . config('app.name') . ' Admin')
...
<div class="w-full">
    ...
    <div class="mb-8 flex items-center gap-3">
        ...
        <div>
            <h1 class="text-3xl font-bold text-slate-900 dark:text-slate-100">Edit Brand</h1>
            <p class="mt-1 text-sm text-slate-600 dark:text-slate-400">Update brand details for {{ $brand->name }}</p>
        </div>
    </div>

    <!-- Form Card -->
    <div class="admin-card p-8">
        <form method="POST" action="{{ route('admin.catalog.brands.update', $brand->id) }}" class="space-y-6">
            @csrf
            @method('PUT')
            
            <div>
                <label for="name" class="block text-sm font-semibold text-slate-700 dark:text-slate-200 mb-2">Brand Name *</label>
                <input 
                    id="name" 
                    name="name" 
                    type="text" 

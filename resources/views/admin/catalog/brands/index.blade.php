...
@section('title', 'Brands - ' . config('app.name') . ' Admin')
...
<div class="w-full">
    ...
    <div class="mb-8 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
            <h1 class="text-3xl font-bold text-slate-900 dark:text-slate-100">Brands</h1>
            <p class="mt-2 text-sm text-slate-600 dark:text-slate-400">Manage brand names and visibility flags</p>
        </div>
        <a 
                ...
                <tbody class="divide-y divide-sf-border/60 dark:divide-[rgba(114,138,221,0.25)]">
                    @forelse ($brands as $brand)
                        <tr class="bg-white/60 text-slate-700 transition-colors hover:bg-sf-accent-primary/5 dark:bg-sf-night-900/60 dark:text-slate-200">
                            <td class="px-6 py-4 font-semibold text-slate-900 dark:text-slate-100">{{ $brand->name }}</td>
                            <td class="px-6 py-4 text-xs text-slate-500 dark:text-slate-400">{{ $brand->slug }}</td>
                            <td class="px-6 py-4 text-center">
                                <span class="inline-flex rounded-full px-2.5 py-1 text-xs font-semibold {{ $brand->is_active ? 'bg-emerald-500/10 text-emerald-500' : 'bg-slate-500/10 text-slate-500 dark:text-slate-400' }}">
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="inline-flex gap-3">
                                    ...
                                    <form method="POST" action="{{ route('admin.catalog.brands.destroy', $brand->id) }}" onsubmit="return confirm('Delete this brand?')" class="inline">
                                        @csrf
                                        @method('DELETE')
                        ...
                        <tr>
                            <td colspan="4" class="px-6 py-12 text-center">
                                <div class="mx-auto max-w-sm">
                                    ...
                                    <h3 class="mt-4 text-sm font-semibold text-slate-900 dark:text-slate-100">No brands yet</h3>
                                    <p class="mt-2 text-sm text-slate-500 dark:text-slate-400">
                                        <a href="{{ route('admin.catalog.brands.create') }}" class="font-semibold text-sf-accent-primary hover:text-sf-accent-secondary">Add your first brand</a>
                                    </p>
                                </div>
                            </td>
                        </tr>

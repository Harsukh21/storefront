...
@section('title', 'Edit Category - ' . config('app.name') . ' Admin')
...
<div class="w-full">
    ...
    <div class="mb-8 flex items-center gap-3">
        ...
        <div>
            <h1 class="text-3xl font-bold text-slate-900 dark:text-slate-100">Edit Category</h1>
            <p class="mt-1 text-sm text-slate-600 dark:text-slate-400">Update category details for {{ $category->name }}</p>
        </div>
    </div>

    <!-- Form Card -->
    <div class="admin-card p-8">
        <form method="POST" action="{{ route('admin.catalog.categories.update', $category->id) }}" class="space-y-6">
            @csrf
            @method('PUT')

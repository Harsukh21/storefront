...
@section('title', 'Edit Admin - ' . config('app.name') . ' Admin')
...
<div class="w-full">
    ...
    <div class="mb-8 flex items-center gap-3">
        ...
        <div>
            <h1 class="text-3xl font-bold text-slate-900 dark:text-slate-100">Edit Admin</h1>
            <p class="mt-1 text-sm text-slate-600 dark:text-slate-400">Update administrator information for {{ $admin->name }}</p>
        </div>
    </div>
        ...
        <form method="POST" action="{{ route('admin.admins.update', $admin->id) }}" class="space-y-6">
            ...
            <div>
                ...
                >
                    ...
                    <option value="manager" @selected(old('role', $admin->role) === 'manager')>Manager</option>
                </select>
                @error('role')
                    <p class="mt-2 text-sm text-red-500 dark:text-red-400">{{ $message }}</p>
                @enderror

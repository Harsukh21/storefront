...
<div class="flex min-h-screen items-center justify-center px-6 py-12">
    <div class="user-card w-full max-w-md p-8">
        <h1 class="text-2xl font-semibold text-slate-900 dark:text-sf-accent-primary">Reset Password</h1>
        <p class="mt-2 text-sm text-slate-600 dark:text-slate-300">Choose a new password to regain access.</p>
        ...
        <form method="POST" action="{{ route('user.password.update') }}" class="mt-6 space-y-5">
            ...
            <button type="submit" class="w-full rounded-full bg-gradient-to-r from-sf-accent-primary to-sf-accent-secondary px-6 py-3 text-sm font-semibold text-white shadow-sf-glow transition hover:opacity-90 dark:text-sf-night-900">Update Password</button>
        </form>
